...
class Zend_View_Helper_Checklist {
	...
	public function checklist($trimestre) {
        ...
        $where = "id in (SELECT DISTINCT programa_id FROM $schema.metas_trimestres where trimestre=$trimestre AND (percentual>0 or not avaliacao_descritiva is null))";
        ...
        foreach ($programas as $programa)
        {
		    ...
		    $select = $db->select();
		    $where = "id in (SELECT DISTINCT projeto_id FROM $schema.metas_trimestres where trimestre=$trimestre AND (percentual>0 or not avaliacao_descritiva is null))";
		    $select->reset('where');
		    $select->where = $where;
		    ...
		    foreach ($projetos as $projeto)
		    {
	            ...
	            $select = $db->select();
			    $where = "id in (SELECT DISTINCT objetivo_especifico_id FROM $schema.metas_trimestres where trimestre=$trimestre AND (percentual>0 or not avaliacao_descritiva is null))";
			    $select->reset('where');
			    $select->where = $where;
	            ...
	            foreach ($objetivos as $objetivo)
	            {
	            	...
	            	$select = $db->select();
				    $where = "id in (SELECT DISTINCT meta_id FROM $schema.metas_trimestres where trimestre=$trimestre AND (percentual>0 or not avaliacao_descritiva is null))";
				    $select->reset('where');
				    $select->where = $where;
